...
- 运行环境：Python 3.10 \~ 3.12（推荐 3.11，与 Docker 镜像一致）
- 前端构建：Node.js 20+（仅 Systemd 原生部署需要在服务器本机安装；Docker 镜像构建会自动使用 Node 构建阶段）
...
Systemd 原生部署不会自动运行前端构建。首次部署、更新前端代码，或清理过 `static/admin/` 后，都需要重新构建管理后台：
...
构建完成后应生成 `static/admin/index.html` 与 `static/admin/assets/`。FastAPI 会在 `/admin` 和 `/admin/` 提供该 React 管理后台。

若服务器发行版自带的 Node 版本低于 20，请改用 NodeSource、nvm 或系统包管理器提供的 Node 20+ 后再执行上述命令。
...
# 返回 200 或带 text/html 响应头说明前端构建产物已被服务端读取
```
...
- 确认云服务商安全组已经放行对应端口
- 确认服务器内部防火墙已经放行端口
- 确认端口没有被其他进程占用：`ss -tunlp | grep 8000`

### 4. 管理后台 404 或空白

- Systemd 部署请确认已执行 `npm ci --prefix frontend && npm run build --prefix frontend`。
- 确认 `static/admin/index.html` 与 `static/admin/assets/` 存在，并且运行服务的用户有读取权限。
- Docker 部署请使用 `sudo docker compose up -d --build` 重新构建镜像，避免继续运行旧镜像。
